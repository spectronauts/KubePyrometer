...
if [ "$IMAGES_LOADED" = "false" ]; then
  echo ">>> Bundled tar not usable (wrong arch?); pulling images from registry"
  SMOKE_IMAGES=("bitnami/kubectl:latest" "busybox:1.36.1")
  for img in "${SMOKE_IMAGES[@]}"; do
    docker pull "$img" 2>/dev/null || echo "WARN: could not pull $img"
  done
  kind load docker-image "${SMOKE_IMAGES[@]}" --name "$KIND_CLUSTER" 2>/dev/null \
    || echo "WARN: kind load docker-image failed; pods will attempt in-cluster pull"
fi
